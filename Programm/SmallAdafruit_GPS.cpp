...
boolean SmallAdafruit_GPS::parse(char *nmea) {
  // do checksum check
...
char SmallAdafruit_GPS::read(void) {
  char c = 0;
...
#if ARDUINO >= 100
SmallAdafruit_GPS::SmallAdafruit_GPS(SoftwareSerial *ser)
#else
...
SmallAdafruit_GPS::SmallAdafruit_GPS(HardwareSerial *ser) {
  common_init();  // Set everything to common state, then...
  gpsHwSerial = ser; // ...override gpsHwSerial with value passed.
}

// Initialization code used by all constructor types
void SmallAdafruit_GPS::common_init(void) {
#if defined(__AVR__) && defined(USE_SW_SERIAL)
...
void SmallAdafruit_GPS::begin(uint32_t baud)
{
#if defined(__AVR__) && defined(USE_SW_SERIAL)
  if(gpsSwSerial) 
...
void SmallAdafruit_GPS::sendCommand(const char *str) {
#if defined(__AVR__) && defined(USE_SW_SERIAL)
  if(gpsSwSerial) 
...
boolean SmallAdafruit_GPS::newNMEAreceived(void) {
  return recvdflag;
}
...
char *SmallAdafruit_GPS::lastNMEA(void) {
  recvdflag = false;
  return (char *)lastline;
}

// read a Hex value and return the decimal equivalent
uint8_t SmallAdafruit_GPS::parseHex(char c) {
    if (c < '0')
      return 0;
    if (c <= '9')
      return c - '0';
